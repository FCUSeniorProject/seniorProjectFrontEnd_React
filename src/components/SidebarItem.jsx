...
function SidebarItem({select , setSelect , title , contents}) {
    ...
    return (
        <div className="mb-7">
            <div className="flex h-auto items-center border-black pl-2 border-b-1 w-[100%]" onClick={() => toggleOpen(!isOpen)}>
                {isOpen ? <SlArrowDown color="#FFFFFF" size="20"/> : <SlArrowRight color="#FFFFFF" size="20"/>}
                <p className="ml-2 cursor-pointer text-3xl font-bold text-white" >{title}</p>
            </div>
            {isOpen && (
